...
prev=None; done=0
print(' kare  onde  reproj_med  reproj_p90  gecen_4px  gecen_20px')
            ...
            if H is not None:
                ...
                for i in range(ns):
                    ...
                    for j in range(min(50,len(mr.pts_prev))):
                        ...
                        errs.append(max(np.hypot(uh-u1,vh-v1),np.hypot(uh2-u2,vh2-v2)))
                    if onde>0 and (best is None or onde>best[0]):
                        e=np.array(errs)
                        best=(onde,np.median(e),np.percentile(e,90),
                              int((e<=4).sum()),int((e<=20).sum()))
                if best:
                    print('%5d %5d %11.2f %11.2f %10d %11d'%(idx,)+'' if False else
                          '%5d %5d %11.2f %11.2f %10d %11d'%(idx,best[0],best[1],best[2],best[3],best[4]))
                else:
                    print('%5d   -- hicbir aday cheirality gecmedi'%idx)
                done+=1
        if done>=10: break
    prev=curr
