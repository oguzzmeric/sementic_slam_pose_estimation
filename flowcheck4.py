"""
flowcheck3.py ile AYNI deney, TEK degisken: akis medyani artik TUM
eslesen noktalar yerine sadece RANSAC inlier noktalari (pose.inlier_mask)
uzerinden hesaplaniyor -- gurultu azaltma denemesi.

    s = c * akis_px(inlier) * Z / f
...
Z_OFFSET = 41.6
WARMUP_END = scale_rec._warmup_end
...
def flow_median(mr, pose):
    """Sadece inlier noktalarindan akis medyani; yetersizse tum noktalar."""
    mask = pose.inlier_mask
    if mask is not None and len(mask) == len(mr.pts_prev) and mask.sum() >= 8:
        idx = mask == 1
        p0, p1 = mr.pts_prev[idx], mr.pts_curr[idx]
    else:
        p0, p1 = mr.pts_prev, mr.pts_curr
    return float(np.median(np.linalg.norm(p1 - p0, axis=1)))
...
print("=" * 66)
print(f"  n={len(rows)} kare  (inlier-filtreli akis)")
print("=" * 66)
